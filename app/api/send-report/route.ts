...
export async function POST(req: NextRequest) {
  try {
    ...
    const fromEmail = process.env.RESEND_FROM_EMAIL || "hello@optisutar.com";
    const notifyEmail = process.env.RESEND_NOTIFY_EMAIL || "info.optisutar@gmail.com";

    // Send report to user
    await resend.emails.send({
      from: `Optisutar SEO Audit <${fromEmail}>`,
      to: email,
    ...
    await resend.emails.send({
      from: `Optisutar Audit Tool <${fromEmail}>`,
      to: notifyEmail,
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f5;padding:40px 20px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr><td style="background:#04071d;border-radius:12px 12px 0 0;padding:24px 32px;">
        ...
        <tr><td style="background:#04071d;padding:32px;border-bottom:1px solid rgba(255,255,255,0.1);">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td>
                ...
                <p style="color:rgba(255,255,255,0.4);font-size:12px;margin:0;">${new Date(report.generatedAt).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</p>
              </td>
              <td align="right">
        ...
        <tr><td style="background:white;padding:24px 32px;">
          <p style="font-size:11px;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 12px;">Summary</p>
          <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #f3f4f6;border-radius:8px;overflow:hidden;">
            <tr>
              <td style="padding:12px 16px;border-bottom:1px solid #f3f4f6;">
                <span style="background:#fee2e2;color:#991b1b;font-size:12px;padding:2px 8px;border-radius:20px;font-weight:600;">${report.summary.totalIssues} Critical</span>
                <span style="background:#fef9c3;color:#854d0e;font-size:12px;padding:2px 8px;border-radius:20px;font-weight:600;margin-left:8px;">${report.summary.totalWarnings} Warnings</span>
                <span style="background:#dcfce7;color:#166534;font-size:12px;padding:2px 8px;border-radius:20px;font-weight:600;margin-left:8px;">${report.summary.totalPassing} Passing</span>
              </td>
            </tr>
        ...
        <tr><td style="background:white;padding:0 32px 24px;">
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#eef2ff;border:2px solid #6366f1;border-radius:12px;">
            <tr><td style="padding:24px;">
              ...
              <p style="font-size:16px;font-weight:700;color:#3730a3;margin:0 0 12px;line-height:1.4;">We know these issues sound familiar — but that's not what's really holding you back.</p>
        ...
        <tr><td style="background:white;padding:0 32px 32px;text-align:center;">
          ...
          <a href="${siteUrl}/contact" style="display:inline-block;background:#6366f1;color:white;font-size:14px;font-weight:600;padding:14px 32px;border-radius:8px;text-decoration:none;margin-top:12px;">Book Your Free Call →</a>
        </td></tr>
